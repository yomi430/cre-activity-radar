...
| Shipped, validate next | NYC PLUTO-snapshot property context | Narrows NYC permits by an exact BBL parcel-use join while keeping work intent separate. Chicago remains unavailable. | Implemented with bundled fixture | Test whether this context changes analyst lead disposition; audit a production-scale PLUTO refresh before claiming current coverage. |
