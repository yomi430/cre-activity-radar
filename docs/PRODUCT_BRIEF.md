...
| Shipped, validate next | NYC PLUTO-snapshot property context | Narrows NYC permits by an exact BBL parcel-use join while keeping work intent separate. Chicago remains unavailable. | Implemented: bundled 8-parcel fixture by default, plus a live full-citywide fetch (858,284 real parcels, `npm run data:fetch:pluto`) added 2026-09-21 | Test whether this context changes analyst lead disposition in practice; the coverage question itself is closed — a production-scale refresh is no longer a blocker, just an opt-in step. |
| Strategic | Selected proprietary CRE datasets and collaboration/CRM integration | Connects a qualified public-data lead to ownership, leasing, market context, and team follow-through. | High, data-dependent | Confirm that enrichment or handoff changes decisions and justifies data/licensing cost. |
