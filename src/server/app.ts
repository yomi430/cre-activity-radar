...
import { adminJobActionSchema, marketSchema } from '../shared/contracts.js';
...
export function createApp(options: { jobs?: AdminJobManager } = {}) {
  ...
  app.get('/api/cells/:cell/brief', (request, response) => { const selected=market(request,response), type=permitType(request,response), selectedCell=cell(request,response); if(!selected||!type||!selectedCell)return; withDatabase(response,(database,datasetId)=>{ const brief=investigationBriefFor(database,selected,selectedCell,type); return brief ? response.json({datasetId,market:selected,data:brief}) : error(response,404,'Cell not found for this market and filter.'); }); });
