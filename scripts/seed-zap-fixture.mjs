...
export function seedZapFixture(db, directory = resolve('data/fixtures')) {
  ...
  `);
  db.exec('DELETE FROM zap_project_cells; DELETE FROM zap_rejections; DELETE FROM zap_project_bbls; DELETE FROM zap_projects; DELETE FROM zap_parcels; DELETE FROM zap_snapshots;');
  const projects = fixture('zap-projects.json', directory), bbls = fixture('zap-bbl.json', directory), parcels = fixture('zap-pluto.json', directory);
  const snapshotId = 'bundled-zap-fixture-v1', retrievedAt = '2026-09-20T00:00:00.000Z', publisherUpdatedAt = '2026-09-19T00:00:00.000Z';
  db.prepare('INSERT INTO zap_snapshots VALUES(?,?,?,?,?,?,?,?,?)').run(snapshotId, retrievedAt, publisherUpdatedAt, publisherUpdatedAt, publisherUpdatedAt, projects.length, bbls.length, parcels.length, 1);
  const knownProjects = new Set();
  const insertProject = db.prepare('INSERT INTO zap_projects VALUES(?,?,?,?,?,?,?,?,0)');
  ...
  const insertParcel = db.prepare('INSERT INTO zap_parcels VALUES(?,?,?,?,?,?)');
  ...
  const insertBbl = db.prepare('INSERT INTO zap_project_bbls VALUES(?,?,?,?,?,?,?,0)'); let orphan = 0;
  ...
  db.prepare(`INSERT INTO zap_project_cells SELECT DISTINCT b.project_id,p.h3_cell,'PARCEL_CENTROID',? FROM zap_project_bbls b JOIN zap_projects project ON project.project_id=b.project_id JOIN zap_parcels p ON p.bbl=b.bbl WHERE b.validated=1 AND p.h3_cell IS NOT NULL`).run(snapshotId);
